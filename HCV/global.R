...
plot_areacomps=function(times,C){
  ...
  p1=ggplot(data=df, aes(x=t+1950, y=y,fill=Stage)) + geom_area() +
    ...
    scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE), lim = c(0, 200000),expand = c(0,0),breaks=seq(0,200000,25000))+
...
plot_twolines=function(times,fout1,fout2){
  ...
  p=ggplot(data=subset(df,times>=65), aes(x=times+1950)) + geom_line(aes(y=y1,colour="baseline"),size=2)+
    geom_line(aes(y=y2,colour="intervention"),size=2)+
    scale_colour_manual("", 
                        breaks = c("baseline", "intervention"),
                        values = c("blue", "red")) +
    xlab("Year") + ylab("Number of people") + ggtitle("Number of current PWID with HCV from 2015 to 2030")+
    
    scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE),expand=c(0,0),lim=c(15000,40000))+
    scale_x_continuous(breaks = c(seq(2015,2030,1)),expand=c(0,0))+
    theme(plot.background = element_rect(fill = "white"),axis.line = element_line(colour = "black"))+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
  #scale_x_discrete(labels=c("2015","2017","2019","2021","2023","2025","2027","2029","2031"))
  return(p)  
