runmodel=function(fitnum){
  ...
  L=coeffsof_v5()
  if (fitnum==1){
    rin=readRDS("www/five_raw.rds") # from five parameter fit
  }else{
    rin=readRDS("www/three_raw.rds") # form three parameter fit
  }
      N=rep(0,720)
      N[1]=560;N[361]=396;N[367]=44
      param_vals=param_setup(rin)
      dum=asode(param_vals,N,L)
      plot_grid(dum$p2, dum$p1, nrow = 2,rel_heights = c(0.45, 0.55))
  }
  
  asode=function(param_vals,N,L){
    library(deSolve)
    ...
    times      = seq(0, 80, by = 1)
    lt=length(times)
    X = ode(y=init, times=times, func=MODEL, parms=parameters,method="ode45")
    X=t(X[,2:721])
    
    scale00=144000/sum(X[L$chronic00,66]);
    scale10=40000/sum(X[L$chronic10,66]); 
    ...
    fout1 = sum(X[L$chronic10,66])/sum(X[361:540,66])
    fout2= (f0[66]+f1[66])/(f0[66]+f1[66]+f2[66]+f3[66]+f4[66]+hcc[66]+dc[66]+lt[66]);
